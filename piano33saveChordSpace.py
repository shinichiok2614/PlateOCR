...
class PianoApp:
    ...
    def apply_history_zoom(self):
        ...
        self.history_canvas.scale("all", 0, 0, zoom, zoom)
    ...
    def change_zoom(self, target, delta):
        if target == "history":
            ...
            if new_zoom < 0.2: new_zoom = 0.2
            if new_zoom > 3.0: new_zoom = 3.0
            self.history_zoom_var.set(round(new_zoom, 2))
            self.apply_history_zoom()

        elif target == "piano":
            new_zoom = self.piano_zoom_var.get() + delta
            if new_zoom < 0.4: new_zoom = 0.4
            if new_zoom > 3.0: new_zoom = 3.0
            self.piano_zoom_var.set(round(new_zoom, 2))
