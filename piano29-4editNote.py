...
class PianoApp:
    def __init__(self, root):
        ...
        # ===============================
        # HỎI TỈ LỆ ZOOM TRƯỚC KHI LOAD UI
        # ===============================
        zoom = simpledialog.askinteger(
            "Zoom khởi động",
            "Nhập tỉ lệ phóng to (%)\nVí dụ: 100 = chuẩn, 120 = to hơn, 80 = nhỏ hơn",
            minvalue=50,
            maxvalue=200
        )
        if zoom is None:
            zoom = 100
        self.zoom_scale = zoom / 100.0
        self.zoom_history_x = zoom / 100.0
        self.zoom_history_y = zoom / 100.0
    ...
    def create_piano_canvas(self):
        # screen_w = int(self.root.winfo_screenwidth() * self.zoom_scale)
        screen_w = int(self.root.winfo_screenwidth())

        total_cycles = 7
        white_notes = 7
        white_width = int((screen_w // (total_cycles * white_notes)) * self.zoom_scale)
        white_height = int(150 * self.zoom_scale)

        black_width = int(white_width * 0.6)
        black_height = int(90 * self.zoom_scale)
        ...
        for c in range(total_cycles):
            ...
            for i in range(white_notes):
                ...
                rect = self.canvas.create_rectangle(
                    x1, 0, x2, white_height, fill="white", outline="black"
                )
                key_id = f"W{c}-{i}"
            ...
            for bp in black_positions:
                ...
                x2 = x1 + black_width
                rect = self.canvas.create_rectangle(
                    x1, 0, x2, black_height, fill="black", outline="black"
                )
                key_id = f"B{c}-{bp}"
                self.key_rects[key_id] = rect
                ...
                self.canvas.tag_bind(rect, "<Button-2>", lambda e, k=key_id: self.edit_key_dialog(k))
                self.canvas.tag_bind(rect, "<Button-3>", lambda e, k=key_id: self.delete_key_from_chord(k))
        # Tự co dãn cửa sổ theo tỉ lệ zoom
        self.root.update_idletasks()
        h = self.root.winfo_height()
        new_w = int(self.root.winfo_screenwidth() * self.zoom_scale)
        new_h = int(400 * self.zoom_scale)  # chỉnh cao thấp tùy ý
    ...
    def on_right_click_history(self, event):
        ...
        menu.add_command(label="✏ Sửa ghi chú", command=lambda: self.edit_note(idx))
        menu.add_command(
            label="🗑️ Xoá ghi chú này",
    ...
    def edit_note(self, idx):
        ...
        # Nếu note là list → chuyển về chuỗi
        ...
        def on_ok():
            text = entry.get().strip()
            if text:
                cur.execute("UPDATE key_presses SET note_text=? WHERE chord_id=? AND note_text=?",(text, chord_id, note))
    ...
    def draw_chord_on_history(self, keys_dict, record_region=False, record_index=None):
        # --- ZOOM CONFIG ---
        zx = self.zoom_history_x if hasattr(self, "zoom_history_x") else 1.0
        zy = self.zoom_history_y if hasattr(self, "zoom_history_y") else 1.0
        ...
        # width theo zoom
        white_width = int((screen_w // (total_cycles * white_notes)) * zx)
        black_width = int(white_width * 0.6)

        # height theo zoom
        white_height = int(20 * zy)
        black_height = int(15 * zy)

        # time -> pixel
        scale_time = 50 * zy

        # --- Vẽ cột cycle ---
        y_top = self.history_y - 2000
        ...
        # max end time
        max_end = max((start + duration for (_h, _f, start, duration) in keys_dict.values()),
                      default=0)

        top_padding = int(6 * zy)
        ...
        for c in range(total_cycles):
            ...
            for i in range(white_notes):
                ...
                # nền trắng
                self.history_canvas.create_rectangle(
                    x1, base_y, x2, base_y + white_height,
                    fill="white", outline="black"
                )

                # Nếu phím được bấm → vẽ màu phím + thanh thời gian + finger trong thanh thời gian
                if key_id in keys_dict:
                    ...
                    bar_top = max(bar_top, chord_top)
                    bar_bottom = min(bar_bottom, base_y)
            ...
            for bp in black_positions:
                ...
                # nền đen
                self.history_canvas.create_rectangle(
                    x1, base_y, x2, base_y + black_height,
