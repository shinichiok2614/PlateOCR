...
class KeyPressDialog(tk.Toplevel):
    def __init__(self, parent, hand="A", finger=1, start_time=0, duration=0.5):
        super().__init__(parent)
        self.title("Chọn ngón tay & thời điểm")
        ...
        tk.Label(self, text="Chọn ngón tay (10 ô):").pack(padx=10, pady=5)
        self.frame_fingers = tk.Frame(self)
        self.frame_fingers.pack(padx=10, pady=5)
        self.finger_buttons = {}

        # Tay trái A1-A5: đảo ngược
        for i in reversed(range(1, 6)):
            btn = tk.Button(
                self.frame_fingers,
                width=4, height=2,
                bg=FINGER_COLORS.get(i, "gray"),
                text=f"A{i}",
                command=lambda h="A", f=i: self.select_finger(h, f)
            )
            btn.pack(side="left", padx=2)
            self.finger_buttons[f"A{i}"] = btn
        # Tay phải B1-B5
        for i in range(1, 6):
            btn = tk.Button(
                self.frame_fingers,
                ...
                bg=FINGER_COLORS.get(i, "gray"),
                text=f"B{i}",
                command=lambda h="B", f=i: self.select_finger(h, f)
            )
            btn.pack(side="left", padx=2)
            self.finger_buttons[f"B{i}"] = btn

        # ==========================
        # Chọn thời điểm bắt đầu
        # ==========================
        tk.Label(self, text="Chọn thời điểm bắt đầu (s):").pack(padx=10, pady=5)
        self.frame_start = tk.Frame(self)
        self.frame_start.pack(padx=10, pady=5)
        self.start_buttons = {}
        ...
        for t in start_times:
            btn = tk.Button(
                ...
                width=4, height=2,
                text=str(t),
                command=lambda s=t: self.select_start_time(s)
            )
            btn.pack(side="left", padx=2)
            self.start_buttons[t] = btn

        # Duration vẫn nhập bằng Entry
    ...
    def select_finger(self, hand, finger):
        self.selected_hand = hand
        self.selected_finger = finger
        for key, btn in self.finger_buttons.items():
            btn.config(relief="raised")
        self.finger_buttons[f"{hand}{finger}"].config(relief="sunken")

    def select_start_time(self, s):
        self.start_time = s
        for t, btn in self.start_buttons.items():
            btn.config(relief="raised")
        self.start_buttons[s].config(relief="sunken")

    def on_ok(self):
        try:
            duration = float(self.entry_duration.get())
            self.result = (self.selected_hand, self.selected_finger, self.start_time, duration)
        except Exception:
            self.result = None
        self.destroy()
...
class PianoApp:
    def __init__(self, root):
        ...
        self.save_button = tk.Button(toolbar, text="💾 Lưu hợp âm", command=self.save_chord, **btn_style)
        ...
        # ZOOM CONTROLS
        # -----------------------------
        zoom_frame = tk.Frame(toolbar, bg="#d5d9dd")
        zoom_frame.pack(side="right", padx=10)

        tk.Label(zoom_frame, text="History Zoom:", bg="#d5d9dd").pack(side="left")

        self.history_zoom_var = tk.DoubleVar(value=1.0)

        tk.Entry(zoom_frame, textvariable=self.history_zoom_var, width=4).pack(side="left", padx=3)

        tk.Button(
            zoom_frame, text="+",
            command=lambda: self.change_zoom("history", +0.1),
            **btn_style
        ).pack(side="left")
        ...
        # Piano Zoom
        tk.Label(zoom_frame, text="Piano Zoom:", bg="#d5d9dd").pack(side="left")

        self.piano_zoom_var = tk.DoubleVar(value=1.0)

        tk.Entry(zoom_frame, textvariable=self.piano_zoom_var, width=4).pack(side="left", padx=3)

        tk.Button(
            zoom_frame, text="+",
            command=lambda: self.change_zoom("piano", +0.1),
            **btn_style
        ).pack(side="left")
        ...
        self.history_canvas = tk.Canvas(self.visual_frame, bg="lightgray", height=200)
        ...
        # ==== Piano Frame ====
        self.piano_frame = tk.Frame(self.main_frame, height=180)
        self.piano_frame.pack(fill="x", side="top", anchor="n")
    ...
    def apply_history_zoom(self):
        # zoom factor
        zoom = self.history_zoom_var.get()
        # chỉ redraw history với zoom mới
        self.redraw_history()

    # -----------------------------
    # Piano zoom: vẽ lại piano theo zoom, giữ trạng thái màu phím
    # -----------------------------
    def apply_piano_zoom(self):
        zoom = self.piano_zoom_var.get()
        screen_w = self.root.winfo_screenwidth()
        total_cycles = 7
        white_notes = 7
        white_width = int(screen_w / (total_cycles * white_notes) * zoom)
        white_height = int(150 * zoom)
        ...
        for c in range(total_cycles):
            ...
            for i in range(white_notes):
                ...
                if key_id in self.selected_keys:
                    _, finger, _, _ = self.selected_keys[key_id]
                    fill_color = FINGER_COLORS.get(finger, "gray")
                elif self.editing_chord_idx is not None and key_id in self.editing_selected_keys:
                    _, finger, _, _ = self.editing_selected_keys[key_id]
                    fill_color = FINGER_COLORS.get(finger, "gray")
                self.key_rects[key_id] = self.canvas.create_rectangle(x1, 0, x2, white_height, fill=fill_color,
                                                                      outline="black")
                self.canvas.tag_bind(self.key_rects[key_id], "<Button-1>", lambda e, k=key_id: self.quick_press_key(k))
                self.canvas.tag_bind(self.key_rects[key_id], "<Button-2>", lambda e, k=key_id: self.edit_key_dialog(k))
                self.canvas.tag_bind(self.key_rects[key_id], "<Button-3>",
                                     lambda e, k=key_id: self.delete_key_from_chord(k))
            ...
            for bp in black_positions:
                ...
                if key_id in self.selected_keys:
                    ...
                    _, finger, _, _ = self.editing_selected_keys[key_id]
                    fill_color = FINGER_COLORS.get(finger, "gray")
                self.key_rects[key_id] = self.canvas.create_rectangle(x1, 0, x2, black_height, fill=fill_color,
                                                                      outline="black")
                self.canvas.tag_bind(self.key_rects[key_id], "<Button-1>", lambda e, k=key_id: self.quick_press_key(k))
                self.canvas.tag_bind(self.key_rects[key_id], "<Button-2>", lambda e, k=key_id: self.edit_key_dialog(k))
                self.canvas.tag_bind(self.key_rects[key_id], "<Button-3>",
        ...
        # resize canvas
        self.canvas.config(width=screen_w, height=white_height)

    def change_zoom(self, target, delta):
        if target == "history":
            new_zoom = self.history_zoom_var.get() + delta
            new_zoom = max(0.2, min(new_zoom, 3.0))
            self.history_zoom_var.set(round(new_zoom, 2))
            self.apply_history_zoom()
    ...
    def create_piano_canvas(self):
        ...
        self.canvas = tk.Canvas(self.piano_frame, width=screen_w, height=white_height)
        ...
        for c in range(total_cycles):
            ...
            for i in range(white_notes):
                ...
                self.canvas.tag_bind(rect, "<Button-2>", lambda e, k=key_id: self.edit_key_dialog(k))
    ...
    def draw_chord_on_history(self, keys_dict, record_region=False, record_index=None):
        ...
        total_cycles = 7
        white_notes = 7
        white_width = (screen_w // (total_cycles * white_notes)) * zoom  # scale width
        white_height = 20 * zoom
        black_height = 15 * zoom
        black_width = int(white_width * 0.6)

        y_top = (self.history_y - 2000) * zoom
        y_bottom = (self.history_y + 2000) * zoom
        ...
        chord_top = self.history_y * zoom
        base_y = chord_top + max_end * scale_time
        ...
        for c in range(total_cycles):
            ...
            for i in range(white_notes):
                ...
                x1 = cycle_offset + i * white_width
                x2 = x1 + white_width
                self.history_canvas.create_rectangle(x1, base_y, x2, base_y + white_height, fill="white",
                                                     outline="black")

                if key_id in keys_dict:
                    hand, finger, start, duration = keys_dict[key_id]
                    color = FINGER_COLORS.get(finger, "gray")
                    self.history_canvas.create_rectangle(x1, base_y, x2, base_y + white_height, fill=color,
                                                         outline="black")

                    bar_bottom = base_y - start * scale_time
                    bar_top = bar_bottom - duration * scale_time
                    if bar_top < chord_top: bar_top = chord_top
                    if bar_bottom > base_y: bar_bottom = base_y
            ...
            for bp in black_positions:
                ...
                x1 = cycle_offset + bp * white_width - black_width // 2
                x2 = x1 + black_width
                self.history_canvas.create_rectangle(x1, base_y, x2, base_y + black_height, fill="black",
                                                     outline="black")

                if key_id in keys_dict:
                    hand, finger, start, duration = keys_dict[key_id]
                    color = FINGER_COLORS.get(finger, "gray")
                    self.history_canvas.create_rectangle(x1, base_y, x2, base_y + black_height, fill=color,
                                                         outline="black")
