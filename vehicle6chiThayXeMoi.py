...
class App:
    def __init__(self, root):
        ...
        self.car_states = {}  # {car_id: {'state':'present/left', 'count':int}}
    ...
    def update_treeview(self, entries):
        # Xe xuất hiện trong frame hiện tại
        current_ids = set([e['car_id'] for e in entries])

        # Kiểm tra xe mới / debounce
        for e in entries:
            cid = e['car_id']
            if cid not in self.car_states:
                self.car_states[cid] = {'state':'present', 'count':1}
            else:
                self.car_states[cid]['state'] = 'present'
                self.car_states[cid]['count'] += 1

        # Xe rời đi (không xuất hiện trong frame hiện tại)
        for cid in list(self.car_states.keys()):
            if cid not in current_ids:
                self.car_states[cid]['count'] -= 1
                if self.car_states[cid]['count'] <= -3:  # debounce rời đi
                    self.car_states[cid]['state'] = 'left'
            else:
                # Reset nếu vẫn xuất hiện
                if self.car_states[cid]['count'] < 0:
                    self.car_states[cid]['count'] = 1
        ...
        # Thêm tất cả entries vào Treeview
        for e in entries:
            cid = e['car_id']
            tag = ''
            if self.car_states[cid]['state'] == 'present' and self.car_states[cid]['count'] >= 2:
                tag = 'new_car'
            elif self.car_states[cid]['state'] == 'left':
                tag = 'left_car'
            self.tree.insert('', tk.END, values=(cid, e['plate_text'] or '', e['ts']), tags=(tag,))

        # Xóa xe rời đi sau 3s
    ...
    def remove_left_cars(self):
        for item_id in self.tree.get_children():
            ...
            car_id = int(vals[0])
            if self.car_states.get(car_id, {}).get('state') == 'left':
        ...
        try:
            while self.running:
                ...
                for car_id,(px1,py1,px2,py2) in matches:
                    ...
                    if plate_text:
                        cv2.putText(frame,str(plate_text),(px1,max(12,py1-20)),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,255),2)
